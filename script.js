class TypingSpeedTesterPro {
    ...
    loadSettings() {
        ...
        const defaults = { difficulty: 'medium', duration: 60, category: 'general', soundEnabled: true };
        
        Object.keys(defaults).forEach(key => {
            if (settings[key] !== undefined) {
                this[key] = settings[key];
                const element = this[key + 'Select'] || this[key + 'Checkbox'];
                if (element) element.value = this[key];
                if (this[key + 'Checkbox']) this[key + 'Checkbox'].checked = this[key];
            }
        });
