class TypingSpeedTesterPro {
    ...
    initializeElements() {
        ...
        // Set initial values to match HTML defaults
        if (this.difficultySelect) this.difficultySelect.value = this.difficulty;
        if (this.durationSelect) this.durationSelect.value = this.duration;
        if (this.categorySelect) this.categorySelect.value = this.category;
        if (this.soundCheckbox) this.soundCheckbox.checked = this.soundEnabled;
        if (this.timerElement) this.timerElement.textContent = this.duration;
    }
    ...
    loadSettings() {
        ...
        // Load difficulty with safety checks
        if (settings.difficulty && this.difficultySelect) {
            this.difficulty = settings.difficulty;
            this.difficultySelect.value = this.difficulty;
        } else if (this.difficultySelect) {
            // Ensure dropdown has a selected value
            this.difficulty = this.difficultySelect.value || 'medium';
        }
        
        // Load duration with safety checks
        if (settings.duration && this.durationSelect) {
            this.duration = settings.duration;
            this.timeLeft = this.duration;
            this.durationSelect.value = this.duration;
            if (this.timerElement) this.timerElement.textContent = this.duration;
        } else if (this.durationSelect) {
            // Ensure dropdown has a selected value
            this.duration = parseInt(this.durationSelect.value) || 60;
            this.timeLeft = this.duration;
            if (this.timerElement) this.timerElement.textContent = this.duration;
        }
        
        // Load category with safety checks
        if (settings.category && this.categorySelect) {
            this.category = settings.category;
            this.categorySelect.value = this.category;
        } else if (this.categorySelect) {
            // Ensure dropdown has a selected value
            this.category = this.categorySelect.value || 'general';
        }
        
        // Load sound setting with safety checks
        if (settings.soundEnabled !== undefined && this.soundCheckbox) {
            this.soundEnabled = settings.soundEnabled;
            this.soundCheckbox.checked = this.soundEnabled;
        } else if (this.soundCheckbox) {
