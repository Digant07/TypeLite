(() =    els('rWpm').textContent='—'; {
  ...
  if (userWpm >= 200) {
    motivationEl.textContent = 'ðŸ† Incredible! You\'re approaching legendary typing speeds!';
    motivationEl.style.background = 'linear-gradient(135deg,rgba(255,215,0,.15),rgba(34,197,94,.15))';
  } else if (userWpm >= 150) {
    motivationEl.textContent = 'ðŸš€ Exceptional! You\'re in the top tier of typists!';
    motivationEl.style.background = 'linear-gradient(135deg,rgba(34,197,94,.12),rgba(59,130,246,.12))';
  } else if (userWpm >= 100) {
    motivationEl.textContent = 'â­ Great job! You\'re typing faster than most professionals!';
    motivationEl.style.background = 'linear-gradient(135deg,rgba(59,130,246,.1),rgba(147,51,234,.1))';
  } else if (userWpm >= 70) {
    motivationEl.textContent = 'ðŸ’ª Good progress! You\'re above average speed!';
    motivationEl.style.background = 'linear-gradient(135deg,rgba(147,51,234,.1),rgba(236,72,153,.1))';
  } else if (userWpm >= 40) {
    motivationEl.textContent = 'ðŸ“ˆ Keep practicing! You\'re on your way to professional speeds!';
  } else {
    motivationEl.textContent = 'ðŸŒ± Great start! Every expert was once a beginner!';
  }
