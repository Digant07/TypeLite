(() => {
  ...
  // leaderboard removed to standalone page
  ...
  const INTERMEDIATE_SENTENCES = [
    ...
    'Mistakes are feedback—review them, adjust, and continue.',
    'Balanced posture, relaxed wrists, and clear breathing improve endurance.',
    'Progress feels invisible—until suddenly, everything is faster.',
  ...
  function generateIntermediatePool(targetWords){
    ...
    // Keep punctuation within words (split on space only)
    return out.join(' ').trim().split(/\s+/);
  }
  ...
  function pickWords(count){
    // depending on difficulty choose generator; fallback to COMMON_WORDS
  ...
  function reset(){
    ...
    // Expert can use fewer larger tokens; beginner may want more simple distribution
  ...
  function finish(){
    ...
    updateStats();
    saveResult();
    // Build result payload for results page
    ...
    const accuracy = totalKeystrokes>0 ? Math.max(0, Math.min(100, Math.round((correctKeystrokes/totalKeystrokes)*100))) : 100;
    // Consistency = 100 - (stddev(samples.wpm)/avg * 100) simplified
    let consistency = 0;
    if(samples.length>1){
  ...
  function handleKey(ev){
    ...
    if(isBackspace){
      ...
      if(charIndex>0){
        ...
        // move to previous word last char
        wordIndex--;
    ...
    if(expectedChar === ' '){
      ...
      // ensure next word exists, otherwise append more
      if(!words[wordIndex]){
  ...
  // Theme
  // Theme handling moved to nav.js (keep minimal fallback if nav.js not loaded yet)
  ...
  difficultyButtons.forEach(btn=>btn.addEventListener('click', (e)=>{
    // Check if this is the expert difficulty and user doesn't have premium access
  ...
  // Session UI now handled in nav.js; keep placeholder for test save
  function updateAuthUI(){/* noop */}

  // Navigation interactions removed (multi-page now)

  // Help dialog events (updated)
  ...
  function saveResult(){
    ...
    const rawWpm = minutes>0 ? Math.round((totalKeystrokes/5)/minutes) : 0;
    const accuracy = totalKeystrokes>0 ? Math.max(0, Math.min(100, Math.round((correctKeystrokes/totalKeystrokes)*100))) : 100;
    // Consistency derived from samples (same logic as finish)
    let consistency = 100;
    if(samples.length>1){
      const avg = samples.reduce((a,b)=>a+b.wpm,0)/samples.length;
      const variance = samples.reduce((a,b)=>a+Math.pow(b.wpm-avg,2),0)/samples.length;
  ...
  window.addEventListener('load', () => {
    // Restore last selected duration if available
    try {
  ...
  // Compact Music Feature Implementation
  ...
  if (customMusicFile) {
    ...
    // Trigger file input when custom option is selected
    const customRadio = document.querySelector('input[value="custom"]');
    if (customRadio) {
  ...
  // Premium System Implementation (User Authentication Based)
  ...
  async function initializePremiumFeatures() {
    ...
    if (isAuthenticated && isPremiumUser()) {
      ...
      // Show premium badges for non-premium users
  ...
  function showLoginRequiredModal(featureName) {
    ...
    modalContent.innerHTML = `
      <h3 style="color: var(--accent); margin-bottom: 1rem;">🔐 Login Required</h3>
  ...
  function showPremiumUpgradeModal(featureName) {
    ...
    modalContent.innerHTML = `
      <h3 style="color: var(--accent); margin-bottom: 1rem;">🌟 Premium Feature</h3>
  ...
  // Update music button click handler to check premium status
  if (musicBtn) {
    musicBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      if (!isPremiumUser()) {
        // For free users, always show the upgrade prompt
        musicDropdown.style.display = musicDropdown.style.display === 'none' ? 'block' : 'none';
      } else {
        // For premium users, show the normal music controls
